...
if __name__ == "__main__":
    ...
    for protein in proteins:
        ...
        if len(biophys) >= 500:
            ...
            kwargs_heavy = {"cpus": 16, "ram": "64GB", "time": "12:00:00"}

            @job(name=f"Optimisation {args.folder}: {protein}_sp80", **kwargs_heavy)
            def optimisation():
                ...
                for target in consistant_targets:
                    try:
                        ...
                        if not os.path.exists(log_path):
                            ...
                            # Stage 1: grid search over pipeline structures
                            _, _, studies_stage1 = magic_now(
                                X=X,
                                y=y,
                                groups=groups,
                                scoring=mcc_scorer,
                                ...
                                tag=f"_{protein}_{target}_s1",
                            )

                            # Find the best pipeline structure averaged across outer folds
                            trial_records = []
                            for study in studies_stage1.values():
                                for trial in study.trials:
                                    trial_records.append({**trial.params, "value": trial.value})

                            df_trials = pd.DataFrame(trial_records)
                            structure_cols = [c for c in df_trials.columns if c != "value"]
                            best_idx = df_trials.groupby(structure_cols)["value"].mean().idxmax()
                            if not isinstance(best_idx, tuple):
                                best_idx = (best_idx,)
                            best_params = dict(zip(structure_cols, best_idx))

                            # Map Optuna params back to magic_now arguments
                            best_algorithm = best_params["algorithm"]
                            best_scaler = best_params.get("scaling_method")
                            pv = best_params.get("add_pvalue_filter", False)
                            bo = best_params.get("add_boruta_filter", False)
                            if pv and bo:
                                best_feature_selector = "optimize"
                            elif pv:
                                best_feature_selector = "pvalue"
                            elif bo:
                                best_feature_selector = "boruta"
                            else:
